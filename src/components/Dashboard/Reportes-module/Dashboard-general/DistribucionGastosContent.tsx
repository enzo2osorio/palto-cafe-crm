...
import { getSinglePeriodRange, type DateRangeType, type CustomDateRange } from '@/utils/date/getDateRangeByType';

interface DistribucionGastosContentProps {
  selectedDateRange: DateRangeType;
  customDateRange: CustomDateRange | null;
}

export const DistribucionGastosContent = ({ selectedDateRange, customDateRange }: DistribucionGastosContentProps) => {
  ...
  useEffect(() => {
    const fetchGastos = async () => {
      setLoading(true);
      try {
        const gastosData = await getDistribucionGastos(selectedDateRange, customDateRange || undefined);
  ...
  const handleCategoryClick = async (e: React.MouseEvent, categoria: string) => {
    ...
    try {
      const subcategoriasData = await getSubcategoriasDeCategoria(categoria, selectedDateRange, customDateRange || undefined);
  ...
  const getPeriodLabel = (dateRange: DateRangeType) => {
    const { label } = getSinglePeriodRange(dateRange, customDateRange || undefined);
    return label;
  };
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <h4 className="font-ui font-semibold text-foreground capitalize">
                        {gasto.categoria}
                      </h4>
                    ...
                    <div className="flex items-center space-x-4">
                      <span className="font-ui font-bold text-lg text-foreground">
                ...
                <div className="space-y-2">
                  ...
                  {[...Array(2)].map((_, i) => (
                    <div key={i} className="flex items-center space-x-4 p-3 rounded-lg bg-muted/10 border border-muted/20 animate-pulse">
                      ...
                      {/* Barra de progreso */}
                      <div className="flex-1">
                        <div className="w-full h-3 bg-muted/40 rounded"></div>
                      </div>
                      
                      {/* Información de montos */}
                      <div className="text-right min-w-[100px] space-y-1">
                        <div className="w-20 h-3 bg-muted/40 rounded"></div>
                        <div className="w-16 h-2 bg-muted/40 rounded"></div>
                        <div className="w-12 h-2 bg-muted/40 rounded"></div>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={subIndex} className="flex items-center space-x-4 p-3 rounded-lg bg-muted/10 border border-muted/20">
                      ...
                      <div className="flex-1">
                        <div className="w-full bg-muted/30 rounded-full h-3">
                          <div
                            className={`h-3 rounded-full ${getCategoryColor(subIndex)}`}
