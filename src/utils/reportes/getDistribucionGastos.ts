...
export const getDistribucionGastos = async (dateRangeType: DateRangeType = 'mensual') => {
  try {
    ...
    const { data: totalRegistros, error: totalError } = await supabase
      .from('registros')
      .select('monto')
    ...
    if (totalRegistros && !totalError) {
      totalGastos = totalRegistros.reduce((sum, r) => sum + r.monto, 0);
    }
...
export const getSubcategoriasDeCategoria = async (categoria: string, dateRangeType: DateRangeType = 'mensual') => {
  try {
    ...
    if (totalRegistros && !totalError) {
      totalGeneral = totalRegistros.reduce((sum, r) => sum + r.monto, 0);
    }
    
    // Query optimizada para obtener registros de la categoría específica
    const { data: registros, error } = await supabase
      .from('registros')
      .select(`
        monto,
